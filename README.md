...
Blobby reads real-time frequency data from the Web Audio API's AnalyserNode. The raw FFT bins are mapped to 64 logarithmic frequency bands, smoothed, and split by stereo channel (left channel drives the left side, right channel drives the right side). Bass frequencies blend at the bottom seam so both sides converge on kicks. The result is drawn as a smooth closed bezier curve around a central circle.
